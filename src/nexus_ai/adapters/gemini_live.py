...
class GeminiLive:
    ...
    """
    def __init__(self, api_key, model=None, input_sample_rate=16000, tools=None, tool_mapping=None):
        self.api_key = api_key
        self.model = model or "gemini-2.5-flash-native-audio-latest"
    ...
    async def start_session(self, audio_input_queue, video_input_queue, text_input_queue, audio_output_callback, audio_interrupt_callback=None):
        config = types.LiveConnectConfig(
            ...
            system_instruction=types.Content(parts=[types.Part(text="You are Nexus_ai, a helpful AI medical assistant. Keep your responses concise and helpful.")]),
            ...
            async def send_audio():
                try:
                    while True:
                        ...
                        logger.info(f"Streaming {len(chunk)} audio bytes from client to Gemini Live (max amplitude: {max_val})")
                            ...
                            if server_content:
                                if server_content.model_turn:
                                    for part in server_content.model_turn.parts:
                                        if part.inline_data:
                                            logger.info(f"Received {len(part.inline_data.data)} audio bytes from Gemini Live, sending to client")
                                ...
                                if server_content.input_transcription and server_content.input_transcription.text:
                                    logger.info(f"User Speech Transcription: {server_content.input_transcription.text}")
                                    await event_queue.put({"type": "user", "text": server_content.input_transcription.text})
                                
                                if server_content.output_transcription and server_content.output_transcription.text:
                                    logger.info(f"Gemini Speech Transcription: {server_content.output_transcription.text}")
                                    ...
                                    if func_name in self.tool_mapping:
                                        try:
                                            ...
                                            if inspect.iscoroutinefunction(tool_func):
                                                ...
                                                result = await loop.run_in_executor(None, lambda arg_dict=args: tool_func(**arg_dict))
                                        except Exception as e:
                                            result = f"Error: {e}"
